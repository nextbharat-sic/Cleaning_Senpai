...
geofence_table = dynamodb.Table('Geofence')
...
def update_region(report_id, new_region):
    ...
    print("after",table.get_item(Key={'Report_ID': report_id}))
...
def lambda_handler(event, context):
    ...
    response = geofence_table.scan(
        ProjectionExpression='Geofence_name'
    )
    geofence_names = []
    for item in response['Items']:
        geofence_name = item.get('Geofence_name')
        if geofence_name:
            geofence_names.append(geofence_name)
    print(geofence_names)
    
    try:
        region_name = 'admin'
        for geofence_id in geofence_names:
            print(geofence_id)
